    ...
    <Container>
      ...
      <Grid>
        ...
        <Row>
          ...
          <IconText>🏅</IconText>
          <HabitName>Best Day</HabitName>

          {weekDays.map((day) => {
            const allDone = habits.length > 0 && habits.every((habit) =>
              habit.doneDates.some((d) => isSameDay(new Date(d), day))
            );
            return (
              <DayLabel key={day.toISOString()}>
                {allDone ? "🏆" : ""}
              </DayLabel>
            );
          })}
